...
function Services() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8004/api/service/viewMyServices/${userId}`).then((res) => {
  ...
  const handleEnableService = () => {
    if (serviceToEnable) {
      axios
        .put(`http://localhost:8004/api/service/enable/${userId}/${serviceToEnable}`)
        .then((response) => {
  ...
  const handleDisableService = () => {
    if (serviceToDisable) {
      axios
        .put(`http://localhost:8004/api/service/disable/${userId}/${serviceToDisable}`)
        .then((response) => {
